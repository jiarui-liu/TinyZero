...
def process_dataset(data, aggregation_target='set', conversation_input_type='short'):
    ...
    if aggregation_target == "conversation":
        assert conversation_input_type == "full", "The input type should have been the full conversation. It doesn't make sense to aggregate the scores over the full conversation when the input is not the full conversation"
...
def split_dataset(train_dataset, test_ratio, seed=42):
    train_split, test_split = train_test_split(train_dataset, test_size=test_ratio, random_state=seed)
...
def process_tomi_data(example, idx, split, template_type='base', data_source='fantom'):
...
def main():
    ...
    parser.add_argument('--local_dir', type=str, default="./data/fantom", 
    ...
    parser.add_argument('--fantom_file', type=str, default='fantom_v1.json',
                       help='Path to rephrased FANTOM JSON file')
    parser.add_argument('--test_ratio', type=float, default=0.05,
                       help='Ratio of test examples')
    ...
    train_dataset, test_dataset = process_dataset(train_dataset), process_dataset(test_dataset)
